...
#define I2C_I2CONCLR_AAC            ((0x04)) // Assert Acknowledge Clear bit 
#define I2C_I2CONCLR_SIC            ((0x08)) // I2C interrupt Clear bit
#define I2C_I2CONCLR_STOC           ((0x10)) // STOP flag Clear bit
#define I2C_I2CONCLR_STAC           ((0x20)) // START flag Clear bit
#define I2C_I2CONCLR_I2ENC          ((0x40)) // I2C interface Disable bit
...
static inline void _send_start(uint8_t port) {
    // Clear the SI bit
    i2c_devs[port]->I2CONCLR = I2C_I2CONCLR_SIC;
    // Set the start bit
    i2c_devs[port]->I2CONSET = I2C_I2CONSET_STA;
    // Wait for complete
...
static inline uint8_t _recv_byte(uint8_t port) {
    ...
    while (!(i2c_devs[port]->I2CONSET & I2C_I2CONSET_SI));
    // Read data
    return (uint8_t)(i2c_devs[port]->I2DAT & I2C_I2DAT_BITMASK);
}
